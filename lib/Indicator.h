...
public:
  ...
  enum Colors {
    ...
    COLOR_GREEN   = RGB(0x2E, 0xFF, 0xB9),
  ...
  uint32_t run() {
    auto currState = ProvState::getInstance()->state;
    ...
    if (m_PrevState != currState) {
      ...
      if (ProvState::getInstance()->buttonPressed) {
        if (millis() - ProvState::getInstance()->buttonPressTime > BUTTON_HOLD_TIME_ACTION)     { return beatLED(COLOR_WHITE,   (int[]){ 100, 100 }); }
        if (millis() - ProvState::getInstance()->buttonPressTime > BUTTON_HOLD_TIME_INDICATION) { return waveLED(COLOR_WHITE,   1000); }
      }
    #endif
     
    switch (currState) {                                          // <off duration>,<on duration>
    case ProvState::MODE_WAIT_WIFI_CONFIG:                        return beatLED(COLOR_BLUE,    (int[]){ 50, 500 });
    case ProvState::MODE_CONNECTING_WIFI:                         return beatLED(COLOR_BLUE,    (int[]){ 50, 50 });  // blink quickly
    case ProvState::MODE_WIFI_CONNECTED_WAIT_APP_CREDENTIALS:     return beatLED(COLOR_GREEN, (int[]){ 1000, 1000 });   
    case ProvState::MODE_SWITCH_TO_STA:                           return beatLED(COLOR_GREEN,    (int[]){ 500, 500 }); 
    case ProvState::MODE_PROV_DONE:                               return beatLED(COLOR_GREEN,   (int[]){ 5000 }); 
...
#if defined(INDICATOR_LED_IS_RGB)
